...
usage() {
  cat <<'EOF'
Usage:
  tools/run_hydrarpc_dedicated_rest_resume.sh [options]

Options:
  --root-outdir <dir>      Root output directory.
                           Default: output/dedicated_rest_<timestamp>
  --checkpoint <dir>       Existing classic checkpoint directory. Required.
  --parallel-jobs <N>      Parallel gem5 jobs within each sweep. Default: 4
...
if [[ -z "$ROOT_OUTDIR" ]]; then
  ROOT_OUTDIR="output/dedicated_rest_$(date +%Y%m%d_%H%M%S)"
fi
mkdir -p "$ROOT_OUTDIR"
...
if [[ "$SKIP_BUILD" -eq 0 ]]; then
  ...
  bash tools/check_gem5_binary_freshness.sh \
    ...
    --label "gem5 binary for dedicated rest resume" \
...
if [[ "$ANY_FAILURES" -ne 0 ]]; then
  echo "Dedicated rest resume complete with failures."
else
  echo "Dedicated rest resume complete."
fi
echo "ROOT=$ROOT_OUTDIR"
