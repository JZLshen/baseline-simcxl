...
usage() {
  cat <<'EOF'
Usage:
  tools/setup_hydrarpc_dedicated_all_disk_image.sh [disk-image]

Inject all dedicated-side guest binaries needed by the dedicated experiment
bundle into one disk image:
  - hydrarpc_dedicated
  - hydrarpc_dedicated_coherent
...
bash "${REPO_ROOT}/tools/setup_hydrarpc_dedicated_coherent_disk_image.sh" "$DISK_IMAGE"
